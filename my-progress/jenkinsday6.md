...
pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE1 = "manju811/backend"
        DOCKER_IMAGE2 = "manju811/frontend"
    }
    stages {
        stage('Git Checkout') {
            steps {
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred') {
                        dir('api') {
                            sh "docker build -t ${env.DOCKER_IMAGE1}:${env.BUILD_NUMBER} ."
                            sh "trivy image --format table -o backend-image-report.html ${env.DOCKER_IMAGE1}:${env.BUILD_NUMBER}"
                            sh "docker push ${env.DOCKER_IMAGE1}:${env.BUILD_NUMBER}"
        ...
        stage('Build-Tag & Push Frontend Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred') {
                        dir('client') {
                             sh "docker build -t ${env.DOCKER_IMAGE2}:${env.BUILD_NUMBER} ."
                             sh "trivy image --format table -o frontend-image-report.html ${env.DOCKER_IMAGE2}:${env.BUILD_NUMBER}"
                             sh "docker push ${env.DOCKER_IMAGE2}:${env.BUILD_NUMBER}"
                        }
                    }
                }
            }
